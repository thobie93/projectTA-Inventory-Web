...
$(document).ready(function(){
	...
	function tampil_data(){
		var kode = $("#kode_jual").val();
		//alert(kode);
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/penjualan/DataDetail",
			data	: "kode="+kode,
			cache	: false,
	...
	$("#cetak").click(function(){
		var kode	= $("#kode_jual").val();
		window.open('<?php echo site_url();?>/penjualan/cetak/'+kode);
		return false();
	});
...
<form name="form" id="form">
    ...
    <table width="100%">
    <tr>    
        <td width="150">Kode Penjualan</td>
        <td width="5">:</td>
        <td><input type="text" name="kode_jual" id="kode_jual" size="12" maxlength="12" readonly="readonly" value="<?php echo $kode_jual;?>" /></td>
    </tr>
    <tr>    
        <td>Tanggal Penjualan</td>
        <td>:</td>
        <td><input type="text" name="tgl" id="tgl"  size="15" maxlength="15" class="easyui-validatebox" data-options="required:true,validType:'length[3,10]'" value="<?php echo $tgl_jual;?>"/></td>
    </tr>
    </table>
    </fieldset>
</td>
